  ...
  const start = useCallback(async () => {
    try {
      ...
      const tick = () => {
        ...
        // Use average of lower frequencies for voice detection
      ...
      // mic not available — level stays at 0
    }
  }, [])
...
/** Waveform bars that react to audio level */
function AudioWaveform({ level }: { level: number }) {
  // Generate 5 bars with staggered heights based on audio level
...
export function OverlayShell() {
  ...
  // --- RECORDING STATE: sleek glassmorphic pill with live waveform ---
  if (isRecording) {
    return (
      <div className="flex h-full w-full items-center justify-center overlay-fade-in">
        <div className="recording-pill flex h-12 items-center gap-3 rounded-full px-4">
          {/* Glow ring behind the mic icon */}
          <div className="relative flex items-center justify-center">
            <div
              className="absolute rounded-full recording-glow"
              style={{
  ...
  // --- PROCESSING STATE: sleek spinner pill ---
  if (isProcessing) {
    return (
      <div className="flex h-full w-full items-center justify-center overlay-fade-in">
        <div className="processing-pill flex h-11 items-center gap-2.5 rounded-full px-5">
          <Loader2 className="h-4 w-4 text-purple-400 animate-spin" />
          <span className="text-[11px] font-medium text-white/50">Processing...</span>
        </div>
      </div>
    )
  }
  ...
  if (trialExpired) {
    return (
      <div className="flex h-full w-full items-center justify-center overlay-fade-in">
        <div className="flex flex-col items-center gap-2 rounded-2xl bg-black/90 border border-white/[0.06] shadow-2xl shadow-black/60 p-3.5 max-w-[210px] backdrop-blur-2xl">
    ...
    return (
      <div className="flex h-full w-full items-center justify-center overlay-fade-in">
        <button
          ...
          className="flex h-11 items-center gap-2 rounded-full bg-black/90 border border-red-500/20 shadow-2xl shadow-black/60 px-4 backdrop-blur-2xl max-w-[300px] cursor-pointer hover:border-red-400/40 transition-all"
        >
          <div className="h-2 w-2 shrink-0 rounded-full bg-red-400 animate-pulse" />
          <span className="text-[11px] font-medium text-red-300/80 truncate">{displayError}</span>
        </button>
      </div>
    )
  }
